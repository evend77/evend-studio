// src/pages/acheteur/CheckoutEvend.tsx
// Checkout e-Vend — style harmonisé avec le panier + encadré info vendeur
// ✅ CORRIGÉ: token lu à chaque fetch (évite 403), guard sur taxes avant paiement (évite 400)
// ✅ CORRIGÉ: affichage nom_boutique au lieu de nom
// ✅ CORRIGÉ: envoi facturation_taxes au backend pour calcul des taxes
...
export default function Checkout({ naviguer, vendeurId }: CheckoutProps) {
  ...
  async function fetchInfos() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/checkout-evend/infos?vendeur_id=${vId}`, {
  ...
  async function recalculerExpedition(province: string) {
    ...
    if (!token || !vId) return;
    try {
      const res = await fetch(`${API_BASE}/checkout-evend/infos?vendeur_id=${vId}&province=${province}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) return;
      const data = await res.json();
  ...
  async function calculerTaxes(
    ...
    try {
      const res = await fetch(`${API_BASE}/checkout-evend/calculer-taxes`, {
        method: 'POST',
  ...
  async function validerPaiement() {
    ...
    if (!modePaiement || !adresseLivraison) return;
    ...
    try {
      ...
      const totalFinal = taxes
        ? parseFloat(taxes.total)
        : parseFloat((sousTotal + frais + pourboire).toFixed(2));
      ...
      const res = await fetch(`${API_BASE}/checkout-evend/creer-commande`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          vendeur_id: vId,
          mode_paiement: modePaiement,
          infos_livraison: adresseLivraison,
  ...
  const peutPayer = modePaiement !== null && adresseLivraison !== null && !aucunPaiement
          ...
          <div style={s.recapCard}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <span style={{ fontSize: '24px', fontWeight: 900, color: C.accentLight }}>
                  {taxesLoading
                    ? '...'
                    : taxes
                      ? `${taxes.total} $`
                      : `${(sousTotal + fraisExp).toFixed(2)} $`}
                </span>
