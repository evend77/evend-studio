// src/pages/gestionnaire/SimplissePlan.tsx
...
const PALIERS: { id: string; limite: number; prix: string; prixNum: number; label: string; desc: string; couleur: string }[] = [
  { id: 'simplisse-25',  limite: 25,  prix: '15,99$', prixNum: 15.99, label: 'Débutant',   desc: 'Parfait pour commencer',           couleur: '#2563eb' },
  { id: 'simplisse-50',  limite: 50,  prix: '18,99$', prixNum: 18.99, label: 'Croissance', desc: 'Pour une boutique en expansion',   couleur: '#7c3aed' },
  { id: 'simplisse-100', limite: 100, prix: '20,99$', prixNum: 20.99, label: 'Pro',         desc: 'Large catalogue de produits',      couleur: '#db2777' },
  { id: 'simplisse-200', limite: 200, prix: '23,99$', prixNum: 23.99, label: 'Entreprise', desc: 'Maximum pour Simplisse',           couleur: '#dc2626' },
];

export default function SimplissePlan({ gestionnaireId }: Props) {
  ...
  useEffect(() => {
    ...
    .then(([gest, count]) => {
      const plan = gest?.plan || 'simplisse-25';
      setPlanActuel(plan);
      setPlanChoisi(plan);
    ...
    <div style={{ maxWidth: 780, margin: '0 auto', padding: '32px 24px', fontFamily: "'Inter',sans-serif" }}>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
          ...
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1a1a1a', margin: 0 }}>Mon plan Simplisse</h1>
      ...
      <div style={{ marginTop: 28, padding: '14px 18px', background: '#f8fafc', borderRadius: 8, fontSize: 12, color: '#888', lineHeight: 1.7 }}>
        ...
        • Le palier maximum du template Simplisse est de 200 produits.
      </div>
    </div>
  );
}