/**
 * ChatVendeurs.tsx — src/pages/admin/ChatVendeurs.tsx
 * Messagerie admin ↔ vendeurs avec pièces jointes S3, chiffrement, polling
...
interface ChatVendeursProps { naviguerVers: (p: string, d?: any) => void; }

export default function ChatVendeurs({ naviguerVers }: ChatVendeursProps) {
  ...
  const chargerConvs = useCallback(async () => {
    try {
      // Route pour admin : /api/messagerie/admin/vendeur/conversations
      let url = `${API}/api/messagerie/admin/vendeur/conversations?limit=100`;
  ...
  const chargerMessages = useCallback(async (convId: number) => {
    try {
      // Route pour admin : /api/messagerie/admin/vendeur/conversations/${convId}/messages
      const r = await fetch(`${API}/api/messagerie/admin/vendeur/conversations/${convId}/messages`, {
  ...
  const envoyerMessage = async () => {
    ...
    try {
      ...
      // Route pour admin : /api/messagerie/admin/vendeur/conversations/${convActive.id}/messages
      const response = await fetch(`${API}/api/messagerie/admin/vendeur/conversations/${convActive.id}/messages`, {
        method: 'POST',
        headers: { 
  ...
  const changerStatut = async (statut: 'ouvert' | 'resolu' | 'ferme') => {
    ...
    try {
      // Route pour admin : /api/messagerie/admin/vendeur/conversations/${convActive.id}/statut
      await fetch(`${API}/api/messagerie/admin/vendeur/conversations/${convActive.id}/statut`, {
        method: 'PUT',
  ...
  const changerPriorite = async (priorite: 'normale' | 'haute' | 'urgente') => {
    if (!convActive) return;
    try {
      // Route pour admin : /api/messagerie/admin/vendeur/conversations/${convActive.id}/priorite
      await fetch(`${API}/api/messagerie/admin/vendeur/conversations/${convActive.id}/priorite`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    ...
    <>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '14px', padding: '24px', width: '480px', boxShadow: '0 16px 48px rgba(0,0,0,0.2)', animation: 'slideUp 0.2s ease' }}>
            ...
            <div style={{ backgroundColor: '#f0f0f0', padding: '12px', borderRadius: '8px', marginBottom: '20px' }}>
              ...
              <p style={{ fontSize: '10px', color: '#888', marginTop: '6px' }}>
                ⚠️ Le message original reste visible ici pour l'administration, mais sera masqué pour le vendeur.
              </p>
            </div>
      ...
      <div style={{ display: 'flex', height: 'calc(100vh - 56px)', backgroundColor: T.bg, overflow: 'hidden' }}>
        ...
        <div style={{ width: '320px', backgroundColor: T.card, borderRight: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          ...
          {/* Header liste */}
          <div style={{ padding: '16px', borderBottom: `2px solid ${T.accent}`, backgroundColor: '#f8fafc' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
              <div>
                <h3 style={{ fontSize: '13px', fontWeight: '800', color: T.accent, margin: 0, textTransform: 'uppercase', letterSpacing: '0.5px' }}>💬 Chat Vendeurs</h3>
            ...
            <input type="text" value={recherche} onChange={e => setRecherche(e.target.value)}
              placeholder="🔍 Rechercher vendeur, boutique, sujet..."
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <p style={{ fontSize: '12px', fontWeight: '700', color: T.text, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.boutique}</p>
        ...
        {convActive ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 20px', borderBottom: `1px solid ${T.border}`, backgroundColor: T.card, display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: '14px', fontWeight: '800', color: T.text, margin: 0 }}>{convActive.boutique} <span style={{ fontSize: '12px', fontWeight: '400', color: T.textLight }}>· {convActive.vendeurNom}</span></p>
                <p style={{ fontSize: '11px', color: T.textLight, margin: 0 }}>{convActive.sujet}</p>
              </div>
            ...
            <div ref={messagesRef} className="msg-scroll" style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '4px', backgroundColor: '#f8fafc' }}>
              ...
              <div style={{ display: 'flex', gap: '16px', padding: '6px 12px', backgroundColor: 'white', borderRadius: '8px', border: `1px solid ${T.border}`, marginBottom: '8px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '10px', fontWeight: '700', color: T.accent }}>👤 Bleu = Admin</span>
                <span style={{ fontSize: '10px', fontWeight: '700', color: '#7c3aed' }}>🛒 Violet = Vendeur</span>
              ...
              {messages.length === 0 ? (
                ...
                <React.Fragment key={groupe.date}>
                  ...
                  {groupe.msgs.map(msg => {
                    ...
                    const couleur = estAdmin ? T.accent : '#7c3aed';
                    ...
                    return (
                      <div key={msg.id} style={{ display: 'flex', flexDirection: estAdmin ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: '8px', marginBottom: '8px', animation: 'fadeIn 0.2s ease' }}>
                        ...
                        <div style={{ maxWidth: '65%', position: 'relative' }}>
                          ...
                          <div style={{ 
                            backgroundColor: estAdmin ? T.accent : 'white', 
                            color: estSupprime ? '#999' : (estAdmin ? 'white' : T.text), 
                            padding: '10px 14px', 
                            borderRadius: estAdmin ? '12px 12px 4px 12px' : '12px 12px 12px 4px', 
                            fontSize: estSupprime ? '11px' : '13px', 
                            lineHeight: '1.5', 
                            boxShadow: '0 1px 4px rgba(0,0,0,0.08)', 
                            border: estAdmin ? 'none' : `1px solid ${T.border}`,
                            fontStyle: estSupprime ? 'italic' : 'normal'
                          }}>
                            {msg.contenu}
                            
                            {/* AFFICHAGE CORRIGÉ DES IMAGES - basé sur l'extension du fichier */}
                                ...
                                {estUneImage(msg.piece_jointe_url) ? (
                                  ...
                                  <a 
                                    ...
                                    style={{ 
                                      ...
                                      gap: '6px', 
                                      fontSize: '11px', 
                                      color: estAdmin ? 'white' : T.accent, 
                                      textDecoration: 'underline',
                                      padding: '4px 8px',
                                      backgroundColor: estAdmin ? 'rgba(255,255,255,0.2)' : '#f0f0f0',
                                      borderRadius: '4px'
                                    }}>
              ...
              {convActive.statut === 'ferme' ? (
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
                  ...
                  <button 
                    ...
                    style={{ 
                      backgroundColor: (texte.trim() || fileInputRef.current?.files?.length) ? T.accent : '#93c5fd', 
                      color: 'white', 
                      border: 'none', 
                      borderRadius: '10px', 
                      padding: '12px 18px', 
