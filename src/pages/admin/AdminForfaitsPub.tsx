...
function ModaleForfait({ plan, onFermer, onSauvegarder }: {
  plan: Partial<PlanPub> | null;
  onFermer: () => void;
  onSauvegarder: (data: { label: string; max_pubs_actives: number | null; prix: number }) => void;
}) {
  const [label, setLabel] = useState(plan?.label || '');
  const [illimite, setIllimite] = useState(plan?.max_pubs_actives === null);
  const [maxPubsActives, setMaxPubsActives] = useState(plan?.max_pubs_actives ?? 10);
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}
      ...
      <div style={{ backgroundColor: 'white', borderRadius: 16, width: '100%', maxWidth: 440, boxShadow: '0 24px 64px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          <label style={{ fontSize: 12, fontWeight: 700, display: 'block', marginBottom: 6 }}>Nombre de pubs actives</label>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 16 }}>
            <input type="number" min={1} value={maxPubsActives} disabled={illimite} onChange={e => setMaxPubsActives(parseInt(e.target.value) || 0)}
        ...
        <div style={{ padding: '14px 24px', borderTop: `1px solid ${THEME.border}`, backgroundColor: '#fafafa', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          ...
          <button
            onClick={() => label.trim() && onSauvegarder({ label: label.trim(), max_pubs_actives: illimite ? null : maxPubsActives, prix })}
            disabled={!label.trim()}
...
function AdminForfaitsPub() {
  const [plans, setPlans] = useState<PlanPub[]>([]);
  const [chargement, setChargement] = useState(true);
  const [modale, setModale] = useState<{ open: boolean; plan: Partial<PlanPub> | null }>({ open: false, plan: null });
  ...
  const charger = async () => {
    setChargement(true);
    try {
      const res = await fetch(`${API_BASE}/plans-pub`, { headers: { Authorization: `Bearer ${token()}` } });
  ...
  const sauvegarder = async (data: { label: string; max_pubs_actives: number | null; prix: number }) => {
    try {
      const estEdition = !!modale.plan?.id;
      const res = await fetch(
        estEdition ? `${API_BASE}/plans-pub/${modale.plan!.id}` : `${API_BASE}/plans-pub`,
        {
  ...
  const supprimer = async (plan: PlanPub) => {
    if (!window.confirm(`Supprimer le forfait "${plan.label}" ?`)) return;
    try {
      const res = await fetch(`${API_BASE}/plans-pub/${plan.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token()}` } });
      const data = await res.json();
  ...
  return (
    <div style={{ padding: '28px 32px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, margin: '0 0 4px', color: THEME.text, textTransform: 'uppercase' }}>📣 Forfaits Pub</h1>
          <p style={{ fontSize: 13, color: THEME.textLight, margin: 0 }}>Gérez les paliers d'abonnement pour l'espace publicitaire sponsors</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
        {plans.map(plan => (
          <div key={plan.id} style={{ background: THEME.card, borderRadius: 12, border: `2px solid ${plan.actif ? THEME.border : '#fca5a5'}`, padding: 20, opacity: plan.actif ? 1 : 0.6 }}>
            ...
            <p style={{ fontSize: 13, color: THEME.text, margin: '0 0 14px' }}>{plan.max_pubs_actives === null ? 'Pubs illimitées' : `${plan.max_pubs_actives} pubs actives`}</p>
...
export default AdminForfaitsPub;