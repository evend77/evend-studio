// src/templates/shared/Panier.tsx
// e-Vend Studio — Panier acheteur — partagé tous templates transactionnels
...
interface ArticlePanier {
  ...
  quantite: number;
  vendeur_id: number;
  vendeur_nom: string; // nom_boutique du collaborateur
...
export default function Panier({ naviguer, gestionnaireId = 0, config = {} }: PanierProps) {
  ...
  async function fetchPlusTard() {
    try {
      const res = await fetch(`${API_BASE}/marketplace/${gestionnaireId}/panier/plus-tard`, {
  ...
  async function mettreDeCote(articleId: number) {
    ...
    try {
      await fetch(`${API_BASE}/marketplace/${gestionnaireId}/panier/${articleId}/plus-tard`, {
        method: "POST",
        credentials: 'include', headers: { Authorization: `Bearer ${getToken()}` },
      });
      await fetchPanier();
  ...
  async function remettreDansPanier(articleId: number) {
    ...
    try {
      await fetch(`${API_BASE}/marketplace/${gestionnaireId}/panier/plus-tard/${articleId}/remettre`, {
        method: "POST",
        credentials: 'include', headers: { Authorization: `Bearer ${getToken()}` },
      });
      await fetchPanier();
  ...
  async function supprimerPlusTard(articleId: number) {
    await fetch(`${API_BASE}/marketplace/${gestionnaireId}/panier/plus-tard/${articleId}`, {
      method: "DELETE",
      credentials: 'include', headers: { Authorization: `Bearer ${getToken()}` },
    });
    await fetchPlusTard();
  ...
  async function fetchPanier() {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/marketplace/${gestionnaireId}/panier`, {
        credentials: 'include', headers: { Authorization: `Bearer ${getToken()}` },
      });
  ...
  async function modifierQte(id: number, qte: number) {
    ...
    setEnCoursQte(id);
    try {
      await fetch(`${API_BASE}/marketplace/${gestionnaireId}/panier/${id}`, {
        method: "PATCH",
        credentials: 'include', headers: { Authorization: `Bearer ${getToken()}`, "Content-Type": "application/json" },
  ...
  async function viderPanier() {
    await fetch(`${API_BASE}/marketplace/${gestionnaireId}/panier`, {
      method: "DELETE",
      credentials: 'include', headers: { Authorization: `Bearer ${getToken()}` },
    });
    setGroupes([]);
  ...
  if (totalArticles === 0) return (
    <div style={s.page}>
      ...
      <div style={s.videWrap}>
        ...
        <button style={s.btnVideContinuer} onClick={() => naviguer("accueil")}>
