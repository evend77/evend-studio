...
router.post('/inscription', async (req, res) => {
  ...
  try {
    ...
    const existe = await pool.query('SELECT id FROM vendeurs WHERE email = $1', [email.toLowerCase()]);
    ...
    const result = await pool.query(
      `INSERT INTO vendeurs (email, mot_de_passe, nom, plan, statut)
    ...
    await pool.query(
      `INSERT INTO sites (vendeur_id, slug, template_id, sous_type, publie, config)
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT id, email, mot_de_passe, nom, plan, statut FROM vendeurs WHERE email = $1`,
      [email.toLowerCase()]
    );
  ...
  try {
    ...
    if (payload.role === 'admin') {
      ...
      const r = await pool.query(
        'SELECT id, email, nom, plan, statut FROM vendeurs WHERE id = $1',
        [payload.id]
      );
      if (r.rows.length === 0) return res.status(401).json({ valid: false });
