    ...
    upload.single('file')(req, res, async (err) => {
        ...
        try {
            ...
            if (ressembleSVG) {
                ...
                if (!verif.ok) {
                    console.warn(`⚠️ Upload rejeté (config site maintenance) : contenu réel non conforme (détecté: ${verif.mimeDetecte || 'inconnu'})`);
            ...
            const s3Key = `config/pagemaintenance/pagemaintenance_${Date.now()}.${extensionFinale}`;
