...
router.get('/acheteurs/:id/stats', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`📊 Stats acheteur ${acheteurId}`);
    
    // Vérifier que l'acheteur accède à ses propres stats
    ...
    try {
      // Vendeurs favoris (table vendeurs_favoris à créer ?)
      // Pour l'instant, valeurs par défaut
      favoris = { totalVendeurs: 0, totalWishlist: 0 };
      
      // Si tu as une table vendeurs_favoris, décommente :
      /*
      ...
      */
      ...
      console.log('⚠️ Erreur stats favoris:', err.message);
    }
    ...
    try {
      ...
      console.log('⚠️ Erreur stats messages:', err.message);
    }

    // =============================================================
    // 5. GRAPHIQUE DÉPENSES 30 JOURS
    // =============================================================
    let depenses30j = [];
    ...
    try {
      ...
      console.log('📊 Dépenses 30j:', depenses30j.length, 'jours');
    } catch (err) {
      console.log('⚠️ Erreur graphique dépenses:', err.message);
    }

    // =============================================================
    // 6. RÉPARTITION STATUTS
    // =============================================================
    ...
    try {
      const result = await pool.query(`
        SELECT 
          CASE 
            ...
            WHEN statut_commande = 'Fulfilled' THEN 'Livrée'
      ...
      repartitionStatuts = result.rows.map(row => ({
        ...
        couleur: 
          row.nom === 'Livrée' ? '#10b981' :
          row.nom === 'En attente' ? '#f59e0b' :
          row.nom === 'Partielle' ? '#3b82f6' :
          row.nom === 'Annulée' ? '#ef4444' : '#8b5cf6'
      }));
      
      console.log('🥧 Répartition statuts:', repartitionStatuts);
    } catch (err) {
      console.log('⚠️ Erreur répartition statuts:', err.message);
    }

    // =============================================================
    // 7. TOP VENDEURS (les plus commandés par l'acheteur)
    // =============================================================
    let topVendeurs = [];
    ...
    try {
      ...
      console.log('🏆 Top vendeurs:', topVendeurs);
    } catch (err) {
      console.log('⚠️ Erreur top vendeurs:', err.message);
    }

    // =============================================================
    // 8. VISITES PRODUITS (simulées si la colonne n'existe pas)
    // =============================================================
    ...
    console.log('✅ Stats acheteur envoyées');
    res.json(stats);
