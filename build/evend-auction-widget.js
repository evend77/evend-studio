(function () {
  ...
  function getProduitId() {
    try {
      ...
      // Methode 1: Variables Liquid injectees par theme.liquid (le plus fiable)
      if (window.evendProductId && window.evendProductTags) {
        ...
        return null; // Page produit mais pas en enchere
      }

      // Methode 2: ShopifyAnalytics
      ...
      // Methode 3: URL /products/ + JSON dans les scripts
  ...
  // ── 2. Charger les donnees de l'enchere depuis l'API ───────────────────────
  ...
  // ── 3. Charger la config admin (layout + options d'affichage) ──────────────
  function chargerConfig(callback) {
    fetch(API_BASE + '/admin/encheres/vendeur-config')
      .then(function(res) { return res.json(); })
      .then(function(data) { callback(null, data); })
      .catch(function() {
        // Config par defaut si API inaccessible
        callback(null, {
  ...
  // ── 4. Charger les mises (pour l'historique et le top bidder) ──────────────
  ...
  // ── 5. Formater le compte a rebours ───────────────────────────────────────
  ...
  // ── 7. Construire le HTML du widget selon le layout ────────────────────────
  ...
  // ── 8. Mettre a jour le timer en temps reel ────────────────────────────────
  ...
  function configurerBoutonMise(enchereData, config) {
    ...
    btn.addEventListener('click', function() {
      ...
      // Verifier si l'acheteur est connecte (Shopify expose window.Shopify.customer si connecte)
      var customer = window.Shopify && window.Shopify.customer;
      if (!customer) {
      ...
      fetch(API_BASE + '/encheres/' + enchereData.id + '/mises', {
        ...
        body: JSON.stringify({
          ...
          acheteur_nom:   (customer.first_name || '') + ' ' + (customer.last_name || ''),
          montant:        montant,
        })
      })
      ...
      .then(function(r) {
        ...
        // Recharger les mises pour mettre a jour le top bidder
  ...
  // ── 10. Rafraichir les donnees (polling) ───────────────────────────────────
  ...
  // ── 11. Trouver le bon endroit pour injecter le widget ────────────────────
  function trouverPointInjection() {
    // Essayer dans l'ordre les selecteurs communs des themes Shopify
    var selecteurs = [
      '[data-product-form]',
      '.product-form',
      '.product__form',
  ...
  function init() {
    ...
    if (document.getElementById('evend-widget')) return;

    var produitId = getProduitId();
    if (!produitId) return; // Pas une page produit avec enchère

    injecterCSS();

    chargerConfig(function(errCfg, config) {
      chargerEnchere(produitId, function(errEnc, enchere) {
        if (errEnc || !enchere) return; // Pas d'enchère active sur ce produit
        ...
        chargerMises(enchere.id, function(errMises, mises) {
          ...
          if (point) {
            ...
            // Fallback: injecter avant la balise </body>
            document.body.appendChild(container);
          }

          // Masquer le bouton "Ajouter au panier" pendant l'enchère
